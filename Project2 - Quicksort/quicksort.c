...
static int lomuto(void *array, int left, int right, size_t elem_sz,
    ...
    char *pivot = (char *)array + (left * elem_sz);
    for (size_t i = left + 1; i <= right; i++){
        if (comp(swaparray + (i*elem_sz), pivot) < 0){
            s++;
            swap(swaparray + (s*elem_sz), swaparray + (i*elem_sz), elem_sz);
        }
    }
    swap(pivot, swaparray + (s * elem_sz), elem_sz);
    return s;
}
